...
$respUrl;
$skey;
function loginQQ($uid, $pwd, $veriCode){
	global $temp_dir;
	global $skey;
	global $qqId;
...
function get_group_name_list_mask($gid)
{
	global $vfwebqq;
	global $temp_dir;
	global $skey;
...
function get_group_chatlog($gid){
	global $qqId;
	global $ptwebQQ;
	global $p_skey;
    global $skey;
    global $vfwebqq;
    $gcode = get_group_name_list_mask($gid);
    ...
    for($i = 0; $i<$dateCount; $i++){
        ...
        if($date->{'ymd'} < $today)
            continue;
        $url = "http://cgi.web2.qq.com/keycgi/top/groupchatlog?ps=10&bs=".$date->{'begseq'}."&es=".$date->{'endseq'}."&gid=".$gcode."&mode=1&vfwebqq={$vfwebqq}&t=1381190020016";
	    global $temp_dir;
	    $ch = curl_init($url);
...
//        curl_setopt($ch, CURLINFO_HEADER_OUT, true); 
        curl_setopt($ch, CURLOPT_REFERER, "http://d.web2.qq.com/proxy.html?v=20110331002&callback=1&id=2");
        $data = curl_exec($ch);
        file_put_contents("./chatLog/qqLog.".$gid.".".$date->{'ymd'}.".txt", $data);
        $retLog .= $data;
    }
}
...
function getAllGroupsChatLog(){
    ...
    foreach($gidsArr as $gid){
        get_group_chatlog($gid);    
    }
}
getAllGroupsChatLog();
?>
